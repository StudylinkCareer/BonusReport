...
10. PACKAGE RESOLUTION (Phase 14c) — package text appears in the Notes
    free-text column. Resolved via ref_service_fee_alias (NFC-lowercase
    matching, 3-strategy: exact line / parens-stripped / substring).
    Result stored in CaseRecord.package_fee_id matching tx_case column.
    Unresolved-looking-like-a-package mentions emit a warning.
...
# Phase 14c: package resolver tuning
PACKAGE_PARTIAL_MIN_LEN = 7  # shortest alias allowed in substring strategy
...
class CaseRecord:
    ...
    package_fee_id: Optional[int]            # Phase 14c — resolved from Notes
...
def _resolve_package_from_notes(
    ...
    """Extract a package_fee_id from the Notes free-text column.

    Package text typically appears on the first line of Notes, sometimes
    embedded in a longer comment. Three matching strategies are tried,
    in order:
      ...
      3. Substring search using aliases of length >= PACKAGE_PARTIAL_MIN_LEN
         characters (avoids matches on short ambiguous shorthands).

    All matching goes through ref_service_fee_alias (populated by
    Phase 14c migration with both bracketed and non-bracketed forms
    observed in real source data).

    Returns:
        (package_fee_id, warning_or_None)

    The warning is non-None only when notes text exists but no package
    could be resolved AND the text contains a likely package mention
    keyword. Otherwise silent — most non-package notes don't trigger
    warnings.
    """
    if not notes_text:
        return None, None
    ...
    # Strategy 3: substring search across the whole normalised text
    pkg_id = resolve_package_fee_by_alias(
        cursor, normalised_full, partial_min_len=PACKAGE_PARTIAL_MIN_LEN
    )
    if pkg_id is not None:
        return pkg_id, None

    # Nothing matched. Warn only if text looks like a package mention.
