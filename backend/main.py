...
REF_LIST_QUERIES: dict[str, str] = {
    ...
    "package_codes":
        ...
        " ORDER BY COALESCE(NULLIF(TRIM(SPLIT_PART(description, ' — ', 1)), ''), service_code)",
    # Phase 16c — Client Type dropdown sourced from ref_client_type.
    # Returns {id, code, name} where code is the constraint-checked value
    # (DU_HOC_FULL etc.) and name is the Vietnamese display label.
    "client_types":
        "SELECT id, "
        "       code, "
...
REF_LIST_STATIC: dict[str, list[str]] = {
    ...
    # Phase 16c — client_types moved to REF_LIST_QUERIES, now sourced from
    # ref_client_type. Returns {id, code, name} so the dropdown can display
    # the Vietnamese label but save the constraint-checked code.
    "course_statuses": [
        "Attending",
        "Enrolled",
        "Cancelled",
