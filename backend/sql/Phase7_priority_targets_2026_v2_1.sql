-- Phase7_priority_targets_2026_v2.sql
--
-- v2 fix: includes direct_target and sub_target columns (NOT NULL in schema)
-- by copying values from the 2025 source rows.
--
...
--      Targets (total/direct/sub) carry forward from the 2025 row.
--      prior_year_owing = 0 (no priority bonuses paid in 2025).
--
...
--    bonus_pct = 0; targets carried forward from the 2025 row;
--    prior_year_owing = 0 (no 2025 priority bonuses to roll forward);
...
-- created_at and updated_at use the table's NOW() defaults.
