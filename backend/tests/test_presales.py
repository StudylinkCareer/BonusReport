...
RATES = {
    # counsellor (role 1) at MEET_LOW = 1,400,000
    ...
    # CO (role 2) at MEET_LOW = 1,000,000
    2: {"id": 2, "office_id": 1, "role_id": 2, "co_sub_subscheme": None,
        "country_bucket": "TARGET", "tier": "MEET_LOW",
        "amount": 1_000_000,
        "effective_from": date(2024, 1, 1), "effective_to": None},
    # presales (role 3) at MEET_LOW — never used in practice (presales
    # gets paid via share, not rate), but defined so the lookup doesn't
    # error if the engine ever asks. Set to 0 so it doesn't accidentally
    # contribute to a presales row's tier.
...
def _make_ctx() -> RunContext:
    return RunContext(
        year=2024, month=6,
        # all three slot staff hit their target
        enrolments_by_staff_office={(10, 1): 2, (20, 1): 2, (30, 1): 2},
        targets_by_staff_office={(10, 1): 2, (20, 1): 2, (30, 1): 2},
        enrolments_by_priority_partner_ytd={100: 10},  # priority target HIT
    )
...
def _make_case(
    ...
    *,
    ...
    return CaseInput(
        ...
        country_id=1, package_service_fee_id=100,  # Superior package
...
def run_scenario(
    ...
    *,
    ...
    for p, e in zip(payments, expected):
        ...
        print(f"  {marker} {p.slot_label:13s} {p.staff_name:14s} "
...
# Counsellor's full bonus would have been:
#   tier 1,400,000 + package 1,000,000 + priority 700,000 = 3,100,000
# (priority = 1,400,000 × 0.5 × 1.0 because Monash target hit)
# 50% of 3,100,000 = 1,550,000
#
# Counsellor row keeps 50%: gross = 3,100,000 - 1,550,000 = 1,550,000
# Presales row receives 50%: gross = 0 + 0 + 0 + 0 + 0 - (-1,550,000) = 1,550,000
# Total paid out = 3,100,000 ✓ (same as if no presales)
...
results.append(run_scenario(
    ...
    case=_make_case(
        1,
        counsellor_staff_id=10, counsellor_name="Truong An",
        presales_staff_id=30, presales_name="Pre Sales Bee",
    ),
    ctx=_make_ctx(),
    ref=_make_ref(),
    expected=[
        # Counsellor (Truong An):
        {"slot_label": "counsellor", "staff_id": 10,
         "tier": 1_400_000, "package": 1_000_000, "priority": 700_000,
         "share": 1_550_000,   # POSITIVE = subtracted
         "gross": 1_550_000},  # 3,100,000 - 1,550,000
        # Presales (Pre Sales Bee):
        {"slot_label": "presales", "staff_id": 30,
         "tier": 0, "package": 0, "priority": 0,
         "share": -1_550_000,  # NEGATIVE = received
         "gross": 1_550_000},  # 0 - (-1,550,000)
    ],
...
# -----------------------------------------------------------------------------
# Scenario 2 — Same person in both slots (Reading A)
# -----------------------------------------------------------------------------

# Person X = counsellor AND presales. Same staff_id on both slots.
# Counsellor row: gross = 1,550,000 (as above)
# Presales row:   gross = 1,550,000 (as above)
# Combined for Person X: 3,100,000 — same as if no presales involved. ✓
...
results.append(run_scenario(
    ...
    case=_make_case(
        2,
        counsellor_staff_id=10, counsellor_name="Truong An",
        presales_staff_id=10, presales_name="Truong An",  # SAME person
    ),
    ctx=_make_ctx(),
    ref=_make_ref(),
    expected=[
...
results.append(run_scenario(
    ...
    expected=[
        {"slot_label": "counsellor", "staff_id": 10,
         "tier": 1_400_000, "package": 1_000_000, "priority": 700_000,
         "share": 0,                  # not applicable
         "gross": 3_100_000},         # full
    ],
